...
// interfaces this server is registered on.  This is used to discard mDNS
// queries that arrive on unrelated interfaces (Linux delivers multicast
// packets to all sockets on the same port due to IP_MULTICAST_ALL=1).
func (s *Server) hasInterface(ifIndex int) bool {
...
func (s *Server) recv4(c *ipv4.PacketConn) {
	...
	for {
		...
		default:
			...
			if ifIndex != 0 && !s.hasInterface(ifIndex) {
				continue
			}
